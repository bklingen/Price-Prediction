...
SubmitDF <- data.frame(test$Id, predicted.lasso)
...
```{r}
new.train <- train %>% filter(!(row_number() %in% c(524, 692, 1171, 1299, 1424)))
...
```{r}
fit.lasso <- glmnet(x=X.sd, y=new.train$SalePrice, alpha=1)
plot(fit.lasso, xvar="lambda", label=TRUE)
```
...
```{r}
fit.lasso.opt <- glmnet(x=X, y=new.train$SalePrice, 
                        lambda=cross.val$lambda.min, 
                        alpha = 1, standardize = TRUE)
coef(fit.lasso.opt)
```
...
SubmitDF <- data.frame(test$Id, predicted.lasso)
colnames(SubmitDF) <- c("Id", "SalePrice")
write.csv(file='../submissions/lasso2.csv', SubmitDF, row.names = FALSE)
```
