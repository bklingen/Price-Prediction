...
```
Let's check how our model performs on the training set.
```{r}
...
```
Three houses (524,692,1171) had extremely high residuals, with 692 & 1171 also having high leverage.  1424 and (to a lesser extent) 1299 stand out, having a cook's D>1.
```{r}
train[c(524,692,1171,1299,1424),]
```
692 is also the highest sale price in the training set, so it makes sense that it's skewing the model somewhat.  524 is a newer house on a fairly large, irregular lot with a low sale price, while 1171 is an older house on a fairly small lot.  1299 and 1424 don't stand out just looking at them, other than being very new & fairly old buildings, respectively.

I decided to drop these five when fitting the model.

The poolQC variable has a factor mis-match between the two sets.  Almost the entire variable is 'none' values, so I decided to drop it.
```{r}
train.temp<-train %>%
  filter(!row_number() %in% c(524,692,1171,1299,1424)) %>% dplyr::select(-c(PoolQC))
test<-dplyr::select(test,-c(PoolQC))
...
```{r}
plot(lm.fAIC.2,which=c(1,2,3,4))
plot(lm.bAIC.2,which=c(1,2,3,4))
```
Let's see how our model works with those points dropped:
```{r}
c(summary(lm.fAIC.2)$fstatistic[1],summary(lm.fAIC.2)$r.squared,dim(as.data.frame(lm.fAIC.2$coefficients))[1])
c(summary(lm.bAIC.2)$fstatistic[1],summary(lm.bAIC.2)$r.squared,dim(as.data.frame(lm.bAIC.2$coefficients))[1])
```
Our R^2 improved by ~.025, which is a decent improvement over ~.90.
...
#print(c(min(fAIC.2.prediction[,1]),max(fAIC.2.prediction[,1])))
```

Now we'll submit the models and see how they perform.
```{r}

df.output<-as.data.frame(cbind(test$Id,bAIC.prediction[,1]))
colnames(df.output)<-c('Id','SalePrice')
write.csv(df.output,'..\\AIC_backward_1.csv')

df.output<-as.data.frame(cbind(test$Id,bAIC.prediction[,1]))
colnames(df.output)<-c('Id','SalePrice')
write.csv(df.output,'..\\AIC_forward_1.csv')

df.output<-as.data.frame(cbind(test$Id,bAIC.2.prediction[,1]))
colnames(df.output)<-c('Id','SalePrice')
write.csv(df.output,'..\\AIC_backward_2.csv')

df.output<-as.data.frame(cbind(test$Id,bAIC.2.prediction[,1]))
colnames(df.output)<-c('Id','SalePrice')
write.csv(df.output,'..\\AIC_forward_2.csv')
```
